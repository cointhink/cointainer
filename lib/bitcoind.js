module.exports = new (function(){
  ...
  this.unprocessed_transactions_since = function(block_hash,cb){
    ...
    this.rpc('listsinceblock', opts, function(resp){
      var ctxs = resp.transactions.filter(function(tx){return tx.confirmations > 0})
      resp.transaction = ctxs
      cb(resp)
    })
  }
  ...
  this.rpc = function(method_name, opts, cb){
    var json_rpc = {method: method_name,
                    params: opts}
    request({ method: 'post',
              uri: this.uri,
              json: json_rpc
