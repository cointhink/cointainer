...
module.exports = new (function(){
  ...
  this.setup = function(_db, _btcd, config) {
    db = _db
    btcd = _btcd
    set_tenant_name(config.tenant.name)
    assert(tenant_name, "Bad or missing tenant name")
    currency = config.bitcoind.currency
    assert(tenant_name, "Bad or missing currency value")
  }
    ...
    db.load_block_hash(function(block_hash){
      btcd.unprocessed_transactions_since(block_hash, function(bitcoin){
        db.transaction(function(){
          ...
          that.transaction_report(total, bitcoin.transactions.length-total)
  ...
  this.account_sync = function(){
    btcd.list_accounts(function(account){
      ...
      if(username){
        db.transaction(function(){
          that.add_user(username,function(receive_address){
  ...
  this.user = function(username, cb){  
    var account = that.username_to_account(username)
    db.account(account,cb)
  }
  ...
  this.balance = function(username, cb){
    ...
    db.transactions(account, function(err, txs){
      if(err){
        ...
        cb({amount:total, currency: currency})
      }
    })
  }

  this.transactions = function(username, cb){
    var account = that.username_to_account(username)
    db.transactions(account, function(err, txs){
      if(err) { cb() } else { cb(txs) }
    })    
  }
})()
