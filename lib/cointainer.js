...
module.exports = new (function(){
        ...
        db.transaction(function(){
          bitcoin.transactions.forEach(function(tx){
            if(that.is_tenant_account(tx.account)){
              ...
              console.error("Warning: Ignoring non-tenant account transaction for "+tx.account)
            }
          })
  ...
  this.is_tenant_account = function(name){
    if(name.indexOf(seperator) == -1) {
      var _tenant_name = name.split(seperator)[0]
  ...
  this.add_user = function(username, cb){
    ...
    db.transaction(function(){
      db.add_account(account, username, function(){
        btcd.account_address(account, function(resp){
          console.log(resp)
          cb(resp)
        })
      })
